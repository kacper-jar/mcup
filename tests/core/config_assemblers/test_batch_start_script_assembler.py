...
from mcup.core.config_assemblers.batch_start_script_assembler import BatchStartScriptAssembler


class TestBatchStartScriptAssembler:
    ...
    def mock_config(self):
        ...
        config.config_file_name = "start.bat"
        return config

    def test_assemble_basic(self, mock_config):
        with patch("builtins.open", mock_open()) as mock_file:
            BatchStartScriptAssembler.assemble("C:/path", mock_config)

            mock_file.assert_called_with("C:/path/./start.bat", "w")
            handle = mock_file()

            args, _ = handle.write.call_args
            content = args[0]
            assert "@echo off" in content
            assert "set \"SERVER_JAR=server.jar\"" in content
            assert "java -Xms1024M -Xmx2048M -jar \"%SERVER_JAR%\" nogui" in content
    ...
    def test_assemble_server_args(self, mock_config):
        ...
        with patch("builtins.open", mock_open()) as mock_file:
            ...
            assert "java -Xms1024M -Xmx2048M  \"%SERVER_JAR%\" nogui" in content
